...
- 最终报告：`docs/深度学习大作业_邹研泽_修订.docx`（含全部图表、Word 公式与参考文献）。
- 报告插图由 `scripts/plot_*` 生成；架构图 / 方法总览组图 / 公式图分别见
  `plot_arch_h.py`、`plot_methods_combined.py`、`plot_formula.py`。
- `docs/handoff.md` 记录了实验诊断与工程改动过程。

---

## 8. 说明

- 模型权重（`*.safetensors` 等）与本地缓存已通过 `.gitignore` 排除，仓库只跟踪可复现所需的
  小文件（`predictions.jsonl`、`trainer_state.json`、配置、指标 CSV、图表）。
- 所有随机性固定 `seed=42`；评测最大生成长度 512。
- 复现压缩包见根目录 `reproducible_code.zip`（含代码 + 复现图表所需的预测/指标，不含模型权重）。
